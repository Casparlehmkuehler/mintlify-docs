...
```bash API
curl -X POST https://api.lyceum.technology/api/v2/external/execution/python/start \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "python_code": "print(\"Hello from Lyceum Cloud!\")",
    "execution_type": "cpu"
  }'
```
</CodeGroup>
  ...
  <Tab title="API with Requirements">
    ...
    curl -X POST https://api.lyceum.technology/api/v2/external/execution/python/start \
      ...
      -d '{
        ...
        "python_requirements": ["requests==2.31.0"],
        "execution_type": "cpu"
      }'
    ```
  </Tab>
