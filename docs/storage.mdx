...
<CardGroup cols={3}>
  <Card title="Persistent Storage" icon="server">
...
<Tabs>
  <Tab title="VS Code" icon="code">
    ...
    <Steps>
      ...
      <Step title="Drag & Drop (Coming Soon)">
  ...
  <Tab title="Web Dashboard" icon="window">
    ### Dashboard Upload
  ...
  <Tab title="API" icon="code">
    ...
    <Note>The `key` parameter is optional. If not provided, the original filename will be used.</Note>
...
<CodeGroup>
  ...
  storage_path = os.environ.get('LYCEUM_STORAGE_PATH', '/lyceum')
...
<Note>The `folder_prefix` parameter is optional and will be prepended to all uploaded filenames.</Note>
